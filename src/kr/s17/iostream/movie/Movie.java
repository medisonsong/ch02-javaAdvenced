...
public class Movie {
	/*
	 * [실습]
	 * 1. 멤버변수: 영화제목(name), 제작연도(create_year/string), 감독(director)
	 * 				배우(actor), 장르(genre), 상영시간(time/int)
	...
	//toString 재정의 (반복적인 작업 줄이기 위해서 해봅시당)
	@Override
	public String toString() {
		return String.format("%s\t%s\t%s\t%s\t%s\t%d\n", name, create_year, director, actor, genre, time); // 문자열 제공+ 문자열 포맷도 제공
